...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    p = np.exp(Z3-np.max(Z3))/np.sum(np.exp(Z3-np.max(Z3)),axis=1,keepdims=True)
    loss = np.sum(-np.log(p[np.arange(N),y]))/N+0.5 * reg * (np.sum(W1 * W1)+np.sum(W2 * W2))
    pass
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    dZ3[np.arange(N),y] -= 1
    dZ3 /= N
    grads['W2'] = A2.T.dot(dZ3) + reg * W2
    grads['b2'] = np.sum(dZ3,axis=0)
    dA2 = dZ3.dot(W2.T)
    dZ2 = dA2 * (Z2>0)
    grads['W1'] = X.T.dot(dZ2) + reg * W1
