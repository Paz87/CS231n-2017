...
# For the moment don't worry about implementing dropout or batch normalization;
#  we will add those features soon.

# ## Initial loss and gradient check

# As a sanity check, run the following to check the initial loss and to gradient check 
# the network both with and without regularization. Do the initial losses seem reasonable?
# 
...
model = FullyConnectedNet([100, 100, 100, 100, 100],
                weight_scale=weight_scale,use_batchnorm =True, dropout = dropout,dtype=np.float64)
i=1
for l in learning_rates:
    
    print(str(i)+'/'+str(len(learning_rates)))
    solver = Solver(model, data,
                    print_every=100, num_epochs=100, batch_size=batch_size,
                    update_rule='adam',
                    optim_config={
                            'learning_rate': l,
                            }
