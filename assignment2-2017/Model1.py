...
convLayersDepths = [3,32,32,64,64,128,128,128] # vgg 16 - convLayersDepths = [3,64,128,256,512,512]
useMaxPooling = [False,True,False,True,False,False,True]
...
convFiltSize = [3]*convLayersNum# vgg 16 - convFiltSize = [3,3,3,3]# For now only square filters allowed
numOfDupConvLayers = [4,4,4,4,4,4,4] # vgg 16 - numOfDupConvLayers = [2,2,3,3,3] # Each conv layer is actually several layers in a row
keep_probs = [0.9,0.3] # First element - keep prob for conv layers and second for FC layers
...
def my_model(X,y,is_training):
    ...
    for i in range(convLayersNum):
        ...
        with tf.variable_scope('conv_'+str(i)+str(numOfDupConvLayers[i]-1)):
            currInput = ConvLayer(currInput, convFiltSize[i],currH,currW,inDepth, convLayersDepths[i+1],useMaxPooling[i],True,is_training,keep_prob = keep_probs[0], activation='relu').output()
...
print('Training')
run_model(sess,y_out,mean_loss,X_train,y_train,20,256,10,train_step,True)
print('Validation')
