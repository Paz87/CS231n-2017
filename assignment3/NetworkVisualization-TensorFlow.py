...
def compute_saliency_maps(X, y, model):
    ...
    ###############################################################################
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels = tf.ones_like(correct_scores), logits=correct_scores))
    grads = tf.abs(tf.gradients(loss,model.image))
    salie = tf.reduce_max(grads,axis = 4)
    saliency = sess.run(salie,feed_dict={model.image: X, model.labels: y })
...
def make_fooling_image(X, target_y, model):
    ...
    ##############################################################################
    classified  = target_y - 1
    i = 0
    while(classified != target_y):
        print(i)
#        print([classified, target_y])
        
        target =  tf.gather_nd(model.classifier, tf.stack((tf.range(X.shape[0]), [target_y]), axis=1))
#        print(loss)
        grads = tf.gradients(target ,model.image)
#        print(X_fooling.shape)
        grads_update,scores = sess.run([grads,model.classifier] ,feed_dict={model.image: X_fooling, model.labels: [target_y] })
        classified = scores[0].argmax()
        print([scores[0,classified],scores[0,target_y]])
        X_fooling[0,:,:,:] += np.squeeze(learning_rate*grads_update/np.linalg.norm(grads_update))
        i+=1
...
def create_class_visualization(target_y, model, **kwargs):
    ...
    ########################################################################
    loss = tf.gather_nd(model.classifier, tf.stack((tf.range(X.shape[0]), [target_y]), axis=1))+l2_reg*(tf.norm(model.image)**2) # scalar loss
