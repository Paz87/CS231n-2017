...
def gram_matrix(features, normalize=True):
    ...
    """
    tensor_shape = tf.shape(features)
    M = tensor_shape[0]*tensor_shape[1]*tensor_shape[2]
    features = tf.reshape(features,[M,tensor_shape[3]])
    gram = tf.matmul(tf.transpose(features),features)
    if(normalize):
        gram /= tf.to_float(M*tensor_shape[3])
    return gram
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in style_layers:
        Ls += style_weights[j]*tf.reduce_sum(tf.squared_difference(gram_matrix(feats[i]),style_targets[j]))
        j+=1
...
def tv_loss(img, tv_weight):
    ...
    a1 = tf.reduce_sum(tf.squared_difference(img[:,1:,:,:],img[:,:-1,:,:]))
    a2 = tf.reduce_sum(tf.squared_difference(img[:,:,1:,:],img[:,:,:-1,:]))
    return tv_weight*(a1+a2)
